...
public class WeightedGraph<V> extends AbstractGraph<V> implements Serializable {
    /** Construct an empty */
    ...
    public MST getMinimumSpanningTree2() {
        return getMinimumSpanningTree(0);
    }
    ...
    public MST getMinimumSpanningTree2(int startingVertex) {
        ...
        while(T.size() < getSize()) {
            ...
            for(int i = 0; i < getSize(); i++) {
                if(adjacencyMatrix[u][i] != null && !T.contains(i) && cost[i] > adjacencyMatrix[u][i]) {
                    cost[i] = adjacencyMatrix[u][i];
                    parent[i] = u;
                }
            }
        }
    ...
    public Double[][] getAdjacencyMatrix() {
        ...
        for(int i = 0; i < getSize(); i++) {
            for(Edge e : neighbors.get(i)) {
                ...
                adjacencyMatrix[e.u][e.v] = we.weight;
            }
        }
